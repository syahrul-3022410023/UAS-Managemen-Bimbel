  ...
  return (
    <>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard icon={CreditCard} label="Pendapatan" value={formatCurrency(totalRevenue)} detail={`${payments.length} transaksi diterima`} tone="income" className="lg:col-span-2" />
        {Object.entries(revenueByMethod).map(([method, amount]) => (
          <KpiCard
            key={method}
            icon={method === "cash" ? Banknote : method === "qris" ? QrCode : method === "transfer" ? CreditCard : MoreHorizontal}
      ...
      <DataTableShell
        icon={CreditCard}
        title="Database Pembayaran"
        totalCount={payments.length}
        totalLabel="transaksi tercatat"
        shownCount={filtered.length}
        searchValue={query}
        onSearchChange={setQuery}
        searchPlaceholder="Cari siswa atau referensi..."
        controls={
          <>
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {filtered.length === 0 && (
                <EmptyStateRow
                  colSpan={6}
                  icon={CreditCard}
                  title={query || filterMethod !== "all" ? "Tidak ada pembayaran yang cocok" : "Belum ada pembayaran"}
                  description={query || filterMethod !== "all" ? "Coba ubah filter atau kata kunci pencarian." : "Data pembayaran lunas akan muncul di sini."}
                />
              )}
            </tbody>
          </table>
