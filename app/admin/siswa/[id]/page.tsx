...
export default async function StudentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const items = [
    ...
    { icon: School, label: "Sekolah & kelas", value: [student.school_name, student.grade].filter(Boolean).join(" - ") || "-" },
    { icon: CalendarDays, label: "Tanggal lahir", value: student.birth_date ?? "-" },
    { icon: UserRound, label: "Orang tua", value: parent ? `${parent.full_name}${parentPhone ? ` - ${parentPhone}` : ""}` : "-" }
  ];

  return (
    <AppShell role={user.role} email={user.email} name={user.name} title="Detail Siswa" activeNav="Siswa">
      <Link href="/admin/siswa" className="mb-6 inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-brand">
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-apple-soft">
          ...
          {items.map(item => {
            ...
            return (
              <div key={item.label} className="flex gap-3 rounded-2xl bg-slate-50 p-4">
                <Icon size={19} className="mt-0.5 text-brand" />
                <div>
        ...
        {packageItem && (
          <div className="mx-6 mb-6 rounded-2xl border border-brand/15 bg-brand/5 p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-brand">Paket Bimbel aktif</p>
            <p className="mt-1 text-lg font-bold text-ink">{packageItem.name}</p>
            <p className="mt-1 text-sm text-slate-600">
              {packageItem.duration_months} bulan - {packageItem.sessions_per_month} sesi/bulan - Rp {new Intl.NumberFormat("id-ID").format(Number(packageItem.price))}
            </p>
          </div>
        )}
      </section>
